...
const RegisterLap: React.FC<RegisterLapProps> = ({
  ...
  const debouncedFetchCompanies = useMemo(() => debounce((searchTerm: string) => {
  ...
  const filteredCompanies = useMemo(() => {
    if (!query || companies.length === 0) return [];

    console.log(companies , "companies inside memo")
    return companies
      .filter((company) =>
  ...
  const handleCompanySelect = useCallback((company: StartupType) => {
    setQuery(company.startup_name);
    setSelectedCompanyId(company.startup_id);
    setValue("organization_id", company.startup_id);
  }, [setValue]);

  const handleFormSubmit: SubmitHandler<FormData> = useCallback((data) => {
