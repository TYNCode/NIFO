    ...
    const fetchUseCaseDetails = async () => {
      try {
        const res = await fetch(
          `https://tyn-server.azurewebsites.net/trends/${useCaseId}/`
        );
    ...
    <div className="bg-white shadow flex flex-col mt-4 gap-4 w-[40vw] h-[85vh] overflow-y-auto overflow-x-clip">
      ...
      >
        ...
        <div className="relative flex flex-row items-center gap-4">
          ...
          <div className="text-sm font-medium text-[#2286C0]">
            Back to Usecases
          </div>
        </div>
        <div className="relative grid grid-cols-2 gap-2">
          <div className="relative text-lg font-semibold">
            {useCaseData.challenge_title}
          </div>
