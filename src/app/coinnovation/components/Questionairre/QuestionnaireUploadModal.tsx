...
const QuestionnaireUploadModal: React.FC<QuestionnaireUploadModalProps> = ({
  ...
  const handleUploadClick = async () => {
    ...
    try {
      ...
      if (!isValidTemplate) {
        toast.info("Invalid template file. Please upload the briefing questionnaire file.");
        setUploading(false);
        return;
      }
      ...
      const structuredData: {
        categories: Record<
          string,
          { questions: string[]; assumedAnswers: string[]; actualAnswers: string[] }
        >;
      } = { categories: {} };
      ...
      jsonData.slice(1).forEach((row: any[]) => {
        ...
        if (siNo !== "" && question === "" && assumedAnswer === "" && actualAnswer === "") {
          ...
          structuredData.categories[currentCategory] = {
            questions: [],
            assumedAnswers: [],
            actualAnswers: [],
          };
      ...
      // 🔥 Dispatching formatted data to update questionnaire
      if (!projectID) {
        toast.error("Missing project ID. Cannot update questionnaire.");
        setUploading(false);
        return;
      }
      ...
      Object.entries(structuredData.categories).forEach(
        ([cat, { questions, assumedAnswers, actualAnswers }]) => {
          formattedCategories[cat] = {
            questions: questions.map((question, i) => ({
              question,
              answer: {
                assumed: assumedAnswers[i] || "",
                actual: actualAnswers[i] || null,
              },
            })),
          };
        }
      ...
      dispatch(updateQuestionnaire({ projectID, updatedCategories: formattedCategories }));
      ...
      setTimeout(() => {
        ...
        toast.success("Questionnaire uploaded and updated!");
        setTimeout(() => {
          setTempFiles([]);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-lg shadow-lg w-[90vw] sm:w-[50vw] flex flex-col px-8 py-6">
        ...
        <hr
          className={`border-[#E3F2FE] w-full mt-2 ${
                  ...
                  <div className="w-full h-[5px] rounded-full bg-gray-200">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        uploadComplete ? "bg-green-500 w-full" : "bg-[#E4E5E7] w-0"
                      }`}
                    ></div>
                  </div>
