...
const ConnectsPage = () => {
  ...
  const { connectionsMade, connectionsReceived } = useAppSelector(
      ...
      <div className="flex flex-row flex-1 w-full">
        ...
        <div className="flex-grow p-6 lg:p-8 overflow-y-auto">
          ...
          <p className="text-gray-600 mb-8">
            Manage your engagement connections and requests
          </p>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {connectionsMade?.length > 0 ? (
                    connectionsMade.map((connection, index) => (
                      <tr key={index} className="border-t">
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() =>
                              handleOpenModal(connection?.requested_org?.startup_name)
                            }
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {connectionsReceived?.length > 0 ? (
                    connectionsReceived.map((connection, index) => (
                      <tr key={index} className="border-t">
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">{connection?.use_case || "N/A"}</td>
                        <td className="px-4 py-3">
                          {connection?.created_at?.split("T")[0] || "N/A"}
                        </td>
                      </tr>
                    ))
                  ) : (
